...
class Covering(set):
    ...
    def crossover(self, other):
        ...
        ob2 = set(other.cover(set(self.graph.edges) -
                              set().union(*(set(b.edges) for b in sb1))))

        ob1 = other - ob2
        sb2 = set(self.cover(set(other.graph.edges) -
                             set().union(*(set(b.edges) for b in ob1))))
...
class CSS(BiGraph):
    def __init__(self, bicliques):
        ...
        for bl, br in bicliques:
            for l in bl:
                for r in br:
                    self.connect(l, r)
            self.base_covering.add(
                BiClique(self.base_covering, (
                    self.nodes[node] for node in (bl | br))))
    ...
    def compress(self):
        ...
        for i in range(self.max_steps):
            ...
            best = population[0]
            print >> sys.stderr, "Best:{0} Gen:{1} ".format(best.cost, i)
...
if __name__ == '__main__':
    ...
    with open(sys.argv[1] + ".old", 'w') as f:
        f.write(source)
        f.close()
